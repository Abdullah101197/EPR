...
if($user_info['user_type'] == 'staff'){
	...
	$advance_data = $AdvancesalaryModel->where('employee_id', $seg_user_id)->first();
	...
	$advance_data = $AdvancesalaryModel->where('salary_type','advance')->where('employee_id', $seg_user_id)->first();
	$loan_data = $AdvancesalaryModel->where('salary_type','loan')->where('employee_id', $seg_user_id)->first();
}
if($idesignations){
...
// var_dump($timesheet_data,$user_info,$employee_detail,$advance_data);die;
?>
<?php
?>
<?php
          ...
          <div class="card-body pb-0">
            <div class="row invoive-info d-pdrint-inline-flex justify-content-md-center">
              <div class="col-md-6">
				...
				<?php
				 $minute = (8 * 30) * 60;
				 $perminut =  (int) @$employee_detail['basic_salary'] / $minute;
				 $perday = (int) @$employee_detail['basic_salary'] / 30 ;
				 $currentsalary = 0;
				?>
				 
				  <!-- <h4>Per minute : <?= (int)  $perminut ?></h4> -->
              </div>
              <div class="col-md-6">
                ...
                <h4 class="text-uppercase text-primary m-l-30"> <strong>
				  ...
				  <!-- <h4>Loan : <?= @$loan_data['monthly_installment'] > 0 ? $loan_data['monthly_installment'] : 0  ?></h4> -->
				 
				  <!-- <h4>This Month Salary : <span id="current_salary"></span></h4> -->
              </div>
            </div>
            <div class="row">
              <div class="col-sm-12 col-md-12">
                <div class="table-responsive">
                    ...
                    <tbody>
                      ...
                      <?php
						...
						if($check_attendance > 0) {
							...
							$currentsalary = $currentsalary + $perday;
						} else {
							// ||leave check
							...
							if($office_shift['monday_in_time'] == '' && $day == 'Monday') {
								...
								$idate_of_joining = strtotime($employee_detail['date_of_joining']);
				  ...
				  <tfoot>
					<tr>
						<td><h4>Basic Salary : <?= @$employee_detail['basic_salary'] ?></h4></td>
						<td> <h4 class="">  Current Salary : <?= (int) @$currentsalary;?>  </h4></td>
						<td>  <h4>Advance : <?= @$advance_data['monthly_installment'] ?></h4></td>
						<td> <h4>Deduction : <?= @$loan_data['monthly_installment'] > 0 ? $loan_data['monthly_installment'] : 0  ?></h4></td>
						<td> <h4>Total Salary : <?= (int) @$currentsalary - (int) @$loan_data['monthly_installment']  ?></h4></td>
					</tr>
