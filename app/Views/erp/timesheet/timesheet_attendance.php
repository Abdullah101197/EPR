...
<div class="card user-profile-list">
  <div id="accordion">
    <div class="card-header">
      ...
      <?php $attributes = array('name' => 'import_sheet', 'id' => 'import_sheet', 'autocomplete' => 'off');?>
        <?php //$hidden = array('token' => $segment_id);?>
        <?= form_open('erp/timesheet/import', $attributes);?>
        <div class="form-body">
          <div class="row">
            <div class="col-md-12">
...
$(document).ready(function() {
  $("#import_sheet").submit(function(e) {
      ...
      var fd = new FormData(this);  // Create FormData object
      ...
      $.ajax({
          url: e.target.action,  // Use the form's action attribute as the URL
          type: "POST",  // POST request
          data: fd,  // Pass FormData object
          ...
          success: function(JSON) {
              ...
              if (JSON.error) {
                  toastr.error(JSON.error);  // Display error message if exists
              } else if (JSON.result) {
                  toastr.success(JSON.result);  // Display success message
              } else {
                  toastr.error('Unexpected response from server.');  // Default error
              }
              setTimeout(function() {
                    location.reload();  // Reload the page
                }, 3000);
              Ladda.stopAll();  // Stop loading animation
          },
