...
if($user_info['user_type'] == 'staff'){
	...
	$get_invoice = $EstimatesModel->where('company_id',$user_info['company_id'])->where('estimates_id', $invoice_id)->first();
	// $invoice_items = $EstimatesitemsModel->where('estimates_id', $invoice_id)->findAll();
} else {
	...
	$get_invoice = $EstimatesModel->where('company_id',$usession['sup_user_id'])->where('estimates_id', $invoice_id)->first();
	// $invoice_items = $EstimatesitemsModel->where('estimates_id', $invoice_id)->findAll();
}
$clients =  $UsersModel->where('user_type','customer')->findAll();
$estimate_brands =  $ConstantsModel->where('type','estimate_brand')->findAll();
$invoice_items = json_decode($get_invoice['estimate_module'], true);
      ...
      <div class="card-body" aria-expanded="true" style="">
        <div class="row m-b-1">
          <div class="col-md-12">
            ...
            <?php echo form_open('erp/estimates/update_estimate', $attributes, $hidden);?>
            <input type="hidden" name="estimates_id" value="<?= $get_invoice['estimates_id'];?>">
            <div class="bg-white">
              <div class="box-block">
                <div class="row">
                  <div class="col-md-3">
                    <div class="form-group">
                      ...
                      <div class="input-group">
                        <input class="form-control" placeholder="<?= lang('Dashboard.xin_estimate_title');?>" name="estimate_title" id="estimate-title" type="text" value="<?= $get_invoice['estimate_title'];?>">
                      </div>
                    </div>
                  </div>
                  <div class="col-md-3">
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input class="form-control" name="ref_no" type="text" value="<?= $get_invoice['ref_no'];?>">
                    </div>
                </div>
                  <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select id="client-dropdown" class="form-control" data-plugin="select_hrm" name="client" data-placeholder="Select Client">
                            <?php foreach($clients as $client) { ?>
                                <option value="<?php echo htmlspecialchars($client['user_id']); ?>"
                                        data-contact="<?php echo htmlspecialchars($client['contact_number']); ?>"
                                        <?php echo ($client['user_id'] == $get_invoice['client_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <?php 
                            foreach ($employees as $key => $employee) { ?>
                                <option value="<?= $employee['user_id'] ?>" <?php echo ($employee['user_id'] == $get_invoice['estimate_attend_to']) ? 'selected' : ''; ?>>
                                <?= $employee['username'] ?>
                            </option>
                        <?php    }
                        ?>  
               ...
               <div class="col-md-3">
                 <div class="form-group">
                   ...
                   <?php 
                     foreach ($employees as $key => $employee) { ?>
                        <option value="<?= $employee['user_id'] ?>" <?php echo ($employee['user_id'] == $get_invoice['added_by']) ? 'selected' : ''; ?>>
                           <?= $employee['username'] ?>
                       </option>
                 <?php    }
                   ?>  
                   ...
                   <?php 
                     foreach ($employees as $key => $employee) { ?>
                        <option value="<?= $employee['user_id'] ?>" <?php if($employee['user_id']==$usession['sup_user_id']):?> selected="selected"<?php endif;?>>
                           <?= $employee['username'] ?> 
                           <?php echo ($employee['user_id'] == $get_invoice['added_by']) ? 'selected' : ''; ?>
                       </option>
                 <?php    }
                   ?>  
             ...
             <div class="col-12">
                ...
                <textarea class="form-control editor" placeholder="Terms and Conditions" name="terms_conditions" cols="30" rows="2"><?= $get_invoice['terms_conditions'] ?? '';?></textarea>
                </div>
            </div> 
             <div class="col-12">
                <div class="form-group">
                ...
                <textarea class="form-control editor" placeholder="General Terms and Conditions" name="general_terms_conditions" cols="30" rows="2"><?= $get_invoice['general_terms_conditions'] ?? '';?></textarea>
                </div>
            </div> 
                </div>
                ...
                <!-- <div id="invoice-footer">
                  ...
                  <hr>
                  <?php
                    // Assuming $get_invoice is an associative array containing the invoice data
                    $split_piping_charge = $get_invoice['split_piping_charge'] ?? 0.00;
                    $split_margin_charge = $get_invoice['split_margin_charge'] ?? 0.00;
                    $duct_piping_charge = $get_invoice['duct_piping_charge'] ?? 0.00;
                    $duct_margin_charge = $get_invoice['duct_margin_charge'] ?? 0.00;
                    $package_piping_charge = $get_invoice['package_piping_charge'] ?? 0.00;
                    $package_margin_charge = $get_invoice['package_margin_charge'] ?? 0.00;
                    $vrf_piping_charge = $get_invoice['vrf_piping_charge'] ?? 0.00;
                    $vrf_margin_charge = $get_invoice['vrf_margin_charge'] ?? 0.00;
                    $cassette_piping_charge = $get_invoice['cassette_piping_charge'] ?? 0.00;
                    $cassette_margin_charge = $get_invoice['cassette_margin_charge'] ?? 0.00;
                    $floor_piping_charge = $get_invoice['floor_piping_charge'] ?? 0.00;
                    $floor_margin_charge = $get_invoice['floor_margin_charge'] ?? 0.00;
                    $vav_box = $get_invoice['vav_box'] ?? 0.00;
                    $extra_pipe = $get_invoice['extra_pipe'] ?? 0.00;
                    $discount = $get_invoice['discount'] ?? 0.00;
                    ?>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="split-piping-rate" value="<?php echo htmlspecialchars($split_piping_charge); ?>" class="form-control" name="split_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="split-margin-split" value="<?php echo htmlspecialchars($split_margin_charge); ?>" class="form-control" name="split_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="duct-piping-rate" value="<?php echo htmlspecialchars($duct_piping_charge); ?>" class="form-control" name="duct_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="duct-margin-split" value="<?php echo htmlspecialchars($duct_margin_charge); ?>" class="form-control" name="duct_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="package-piping-rate" value="<?php echo htmlspecialchars($package_piping_charge); ?>" class="form-control" name="package_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="package-margin-split" value="<?php echo htmlspecialchars($package_margin_charge); ?>" class="form-control" name="package_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="vrf-piping-rate" value="<?php echo htmlspecialchars($vrf_piping_charge); ?>" class="form-control" name="vrf_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="vrf-margin-split" value="<?php echo htmlspecialchars($vrf_margin_charge); ?>" class="form-control" name="vrf_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="cassette-piping-rate" value="<?php echo htmlspecialchars($cassette_piping_charge); ?>" class="form-control" name="cassette_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="cassette-margin-split" value="<?php echo htmlspecialchars($cassette_margin_charge); ?>" class="form-control" name="cassette_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="floor-piping-rate" value="<?php echo htmlspecialchars($floor_piping_charge); ?>" class="form-control" name="floor_piping_charge" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="floor-margin-split" value="<?php echo htmlspecialchars($floor_margin_charge); ?>" class="form-control" name="floor_margin_charge" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <hr>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input id="vav-box" value="<?php echo htmlspecialchars($vav_box); ?>" class="form-control" name="vav_box" type="number" step="any">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="extra-pipe">
                                    Extra Pipe<span class="text-danger">*</span>
                                </label>
                                <input id="extra-pipe" value="<?php echo htmlspecialchars($extra_pipe); ?>" class="form-control" name="extra_pipe" type="number" step="any">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="discountInput"><strong>Discount:</strong></label>
                                <input name="discount" type="number" value="<?php echo htmlspecialchars($discount); ?>" class="form-control" id="discountInput" placeholder="0.00" step="any">
                            </div>
                        </div>
                    </div>
...
<script>
  var existingData = <?php echo json_encode($invoice_items); ?>;
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to generate modules and items from existing data
    function populateExistingData(data) {
      var moduleList = document.getElementById('module-list');
      var modules = Object.values(data).filter(item => item !== data.overall_totals);

      modules.forEach(function(module, moduleIndex) {
          var moduleElement = createModuleElement(module, moduleIndex);
          moduleList.appendChild(moduleElement);
          ...
          module.items.forEach(function(item) {
              addItem(moduleElement.querySelector('.item-list'), moduleElement, moduleIndex, item);
          });
          module.custom_items.forEach(function(item) {
              addCustomItem(moduleElement.querySelector('.custom-item-list'), moduleElement, moduleIndex, item);
          });

          // Handle pre-selection of model codes and machine types
          moduleElement.querySelectorAll('.item-row').forEach(function(itemElement, itemIndex) {
              var modelCodeSelect = itemElement.querySelector('select[name="model_code[]"]');
              var brandSelect = itemElement.querySelector('select[name="brand[]"]');
              var machineTypeSelect = itemElement.querySelector('select[name="machine_type_id[]"]');

              if (modelCodeSelect && brandSelect) {
                  var currentItem = module.items[itemIndex];

                  // Set the selected brand and trigger model code fetch
                  var brandId = brandSelect.value;
                  if (brandId) {
                      fetchModelCodesByBrand(brandId, itemElement, currentItem.model_code);
                  }

                  // Attach event listener to update model codes when brand is changed
                  brandSelect.addEventListener('change', function(e) {
                      var selectedBrandId = e.target.value;
                      fetchModelCodesByBrand(selectedBrandId, itemElement);
                  });
                if (currentItem.model_code) {
                  fetchProductDetails(currentItem.model_code, itemElement);
                }
                
    
                  // Set the selected machine type
                  if (currentItem.machine_type_id && machineTypeSelect) {
                      machineTypeSelect.value = currentItem.machine_type_id;
                      $(machineTypeSelect).trigger('change'); // Ensure Select2 is updated
                  }
              }
          });
      });
      ...
      var moduleTemplate = `
          <div class="module">
              ...
              <div class="module-header">
                  <div class="form-group">
                      <label for="module_name">Module Name</label>
                      <input type="text" class="form-control module-name-input" name="module_name[]" value="${module.module_name}" placeholder="Enter module name">
                  </div>
                  <button type="button" class="btn btn-sm btn-outline-danger remove-module">
                      <span class="fa fa-trash"></span> Remove Module
                  </button>
    ...
    function addCustomItem(itemList, moduleElement, moduleIndex, item = {}) {
        // console.log(item)
        ...
        var itemTemplate = `
            <div class="row custom-item-row mt-2 mb-2">
                ...
                <div class="form-group mb-1 col-sm-12 col-md-2">
                    ...
                    <input type="text" class="form-control" value="${item.name || ''}" name="custom_name[]" placeholder="Name">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-3">
                    ${labels ? '<label for="description">Description</label>' : ''}
                    <input type="text" class="form-control" value="${item.description || ''}" name="custom_description[]" placeholder="Description">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-2">
                    ${labels ? '<label for="qty">Qty</label>' : ''}
                    <input type="number" min="1" class="form-control" value="${item.qty || ''}" name="custom_qty[]" value="1">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-2">
                    ${labels ? '<label for="price">Price</label>' : ''}
                    <input type="number" class="form-control" min="1" value="${item.price || ''}" name="custom_price[]" value="0" placeholder="Price">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-2">
                    ${labels ? '<label for="total_price">Total Price</label>' : ''}
                    <input type="number" readonly class="form-control" value="${item.total_price || ''}" name="custom_total_price[]" value="0" placeholder="Total Price">
                </div>
    ...
    function addItem(itemList, moduleElement, moduleIndex, item = {}) {
        var labels = itemList.children.length === 0;
        var itemTemplate = `
            <div class="row item-row">
                ...
                <div class="form-group mb-1 col-sm-12 col-md-1">
                    ...
                    <input type="text" class="form-control qty_hrs" name="location[]" value="${item.location || ''}" placeholder="Location">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-1">
                    ${labels ? '<label for="qty_hrs"><?= lang('Invoices.xin_title_area');?></label>' : ''}
                    <input type="text" class="form-control qty_hrs" name="area[]" value="${item.area || ''}" placeholder="Area">
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-2">
                    ...
                    <select class="form-control model-brand-select" data-plugin="select_hrm" data-placeholder="Select Brand" name="brand[]">
                        ...
                        <option value="<?php echo $estimates_product['constants_id']?>" ${item.brand == <?php echo $estimates_product['constants_id']?> ? 'selected' : ''}><?php echo $estimates_product['category_name']?></option>
                        <?php } ?>
                    </select>
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-2">
                ...
                <div class="skin skin-flat form-group mb-1 col-sm-12 col-md-1">
                    ...
                    <input class="form-control unit_price" type="text" name="capacity[]" value="${item.capacity || '0'}" readonly/>
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-1">
                    ${labels ? '<label for="qty_hrs"><?= lang('Invoices.xin_title_description');?></label>' : ''}
                    <input type="text" class="form-control qty_hrs" name="description[]" value="${item.description || ''}" readonly>
                    <input type="hidden" class="form-control qty_hrs" name="price[]" value="${item.price || ''}" readonly>
                </div>
                <div class="form-group mb-1 col-sm-12 col-md-1">
                    ${labels ? '<label for="qty_hrs"><?= lang('Invoices.xin_title_qty_hrs');?></label>' : ''}
                    <input type="text" class="form-control qty_hrs" name="qty[]" value="${item.qty || '1'}">
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        ${labels ? '<label for="first_name"><?= lang('Inventory.xin_machine_type');?> <span class="text-danger">*</span></label>' : ''}
                        <input type="text" class="form-control machine-type-select" name="machine_type_name[]" value="${item.machine_type_name || ''}" readonly>
                        <input type="hidden" class="form-control machine-type-id" name="machine_type_id[]" value="${item.machine_type_id || ''}">
                    </div>
                </div>
                <div class="form-group col-sm-12 col-md-1 text-xs-center mt-2">
                    <button type="button" class="btn icon-btn btn-sm btn-outline-danger remove-item">
    ...
    function fetchProductDetails(productId, itemElement) {
        ...
        // Remove 'edit-estimate/' from the current URL if it exists
        var baseUrl = currentUrl.split('edit-estimate/')[0];

        // Construct the URL for the fetch request
        var url = baseUrl + 'estimate-product-details/' + productId;
        ...
        fetch(url)
            ...
            .then(data => {
                if (data) {
                    ...
                    if (machineTypeIdInput && data.machine_type_id) {
                        machineTypeIdInput.value = data.machine_type_id || '';
                    }
                    if (machineTypeNameInput && data.machine_type_name) {
                        machineTypeNameInput.value = data.machine_type_name || '';
                    }

                    // Ensure Select2 is updated
                    var machineTypeSelect = itemElement.querySelector('select[name="machine_type_id[]"]');
                    if (machineTypeSelect && data.machine_type_id) {
                        machineTypeSelect.value = data.machine_type_id;
        ...
        function fetchModelCodesByBrand(brandId, itemElement, selectedModelCode = null) {
            var currentUrl = window.location.href;
            ...
            // Construct the URL for the fetch request
            var url = baseUrl + 'fetch-model-codes-by-brand/' + brandId;

            fetch(url)
                .then(response => response.json())
                .then(data => {
                    var modelCodeSelect = itemElement.querySelector('select[name="model_code[]"]');
                    modelCodeSelect.innerHTML = '<option value="">Choose</option>'; // Clear previous options and add a default
                    ...
                    data.forEach(function(model) {
                        ...
                        // If this is the selected model code, set it as selected
                        if (selectedModelCode && model.estimates_products_id === selectedModelCode) {
                    ...
                    // Trigger change event to update Select2 (if applicable)
                    if (selectedModelCode) {
                        $(modelCodeSelect).trigger('change');
        ...
        setInterval(updateOverallTotals, 3000);
});
