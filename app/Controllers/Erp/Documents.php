...
class Documents extends BaseController
{
	...
	public function document_view()
	{
		...
		$isegment_val = $OfficialdocumentsModel->where('document_id', $ifield_id)->first();
		if (!$isegment_val) {
			$session->setFlashdata('unauthorized_module', lang('Dashboard.xin_error_unauthorized_module'));
			return redirect()->to(site_url('erp/desk'));
		}
		...
		$data['subview'] = view('erp/system_documents/document_view', $data);
	...
	public function official_documents_list()
	{
		...
		foreach ($get_data as $r) {
			...
			if (in_array('officialfile3', staff_role_resource()) || $user_info['user_type'] == 'company') {
				$view = '<span data-toggle="tooltip" data-placement="top" data-state="primary" title="' . lang('Main.xin_view_details') . '"><a href="' . site_url() . 'erp/document-view/' . uencode($r['document_id']) . '"><button type="button" class="btn icon-btn btn-sm btn-light-primary waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>';
			} else {
				$view = '';
			}
			...
			$data[] = array(
				...
				$r['document_type'],
				$r['license_name'],
				($r['document_category'] == 0) ? "System" : "Official",
				$r['license_no'],
				$r['expiry_date'],
				$download_link
			);
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if (!$this->validate($rules)) {
				$ruleErrors = [
					...
					"document_file" => $validation->getError('document_file'),
					"document_category" => $validation->getError('document_category')
				...
				$document_category = $this->request->getPost('document_category', FILTER_SANITIZE_STRING);
