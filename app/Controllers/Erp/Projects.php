...
class Projects extends BaseController {
	...
	public function client_profile_estimates_list() {
		$session = \Config\Services::session();
		$usession = $session->get('sup_username');
		...
		// Decoding the client ID from the request
		$client_id = udecode($this->request->getVar('client_id', FILTER_SANITIZE_STRING));
	
		// Fetch estimates for the client, ordering by estimates_id (latest first)
		$get_data = $EstimatesModel->where('client_id', $client_id)->orderBy('created_at', 'DESC')->findAll();
		$UsersModel = new UsersModel();
		$data = [];
		foreach($get_data as $estimate) {
			$created_by = $UsersModel->where('user_id',$estimate['added_by'])->first();
			...
			$data[] = [
				'invoice_number' => $estimate['estimate_number'] . ' <a href="'.site_url() . 'erp/estimate-detail/' . uencode($estimate['estimates_id']).'" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i> View</a>',
				'title' => $estimate['estimate_title'],
				'attend_to' => $estimate['estimate_attend_to'],
				'status' => $status,
				'created_by' => $created_by['first_name'] . ' ' . $created_by['last_name'],
				'created_at' => date('d M, Y', $estimate['created_at']),
			];
		}
	...
	public function estimates_list() {
		...
		$data = [];
		foreach($get_invoices as $estimate) {
			$created_by = $UsersModel->where('user_id', $estimate['added_by'])->first();
			$client = $UsersModel->where('user_id', $estimate['client_id'])->first();
	
			$client_name = trim(($client['first_name'] ?? '') . ' ' . ($client['last_name'] ?? ''));
			$created_by_name = trim(($created_by['first_name'] ?? '') . ' ' . ($created_by['last_name'] ?? ''));
	
			$status = $estimate['status'] == 0 
				? '<span class="badge badge-pill badge-warning">Pending</span>' 
				: '<span class="badge badge-pill badge-success">Approved</span>';
	
			// Build action buttons with conditions
			$actions = '<div class="task-board m-0 float-right">';
			$actions .= '<a href="'.site_url() . 'erp/estimate-detail/' . uencode($estimate['estimates_id']).'" class="btn btn-primary"><i class="fas fa-eye m-0"></i></a>';
			$actions .= '<div class="ml-2 dropdown-secondary dropdown d-inline">';
			$actions .= '<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-bars"></i></button>';
			$actions .= '<div class="dropdown-menu dropdown-menu-right" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">';
	
			// Add print option
			$actions .= '<a class="dropdown-item" href="'.site_url() . 'erp/print-estimate/' . uencode($estimate['estimates_id']).'"><i class="fas fa-download mr-2"></i>' . lang('Main.xin_download_estimate') . '</a>';
			$actions .= '<div class="dropdown-divider"></div>';
	
			// Add edit option based on status and role/resource
			if ($estimate['status'] == 0) {
				if (in_array('estimate4', staff_role_resource()) || $user_info['user_type'] == 'company') {
					$actions .= '<a class="dropdown-item" href="'.site_url() . 'erp/edit-estimate/' . uencode($estimate['estimates_id']).'"><i class="fas fa-edit mr-2"></i>' . lang('Main.xin_edit_estimate') . '</a>';
				}
			}
	
			// Add delete option based on role/resource
			if (in_array('estimate5', staff_role_resource()) || $user_info['user_type'] == 'company') {
				$actions .= '<a href="#!" class="dropdown-item delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'.uencode($estimate['estimates_id']).'"><i class="feather icon-trash-2"></i>' . lang('Main.xin_remove_estimate') . '</a>';
			}
