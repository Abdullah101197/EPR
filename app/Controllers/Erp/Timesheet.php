	...
	public function import()
{
    ...
    $Return = ['result' => '', 'error' => ''];  // Initialize result and error keys
    ...
    if ($file && $file->isValid() && !$file->hasMoved()) {
        ...
        if ($file->getExtension() != 'xlsx') {
            $Return['error'] = 'Invalid file format. Please upload an Excel (.xlsx) file.';
        ...
        try {
            ...
            if (empty($sheetData) || count($sheetData[0]) < 4) {
                $Return['error'] = 'Invalid file format. The file must contain at least 4 columns: Employee Name, Contact Number, Clock-in, Clock-out.';
                return $this->response->setJSON($Return);
            }
            ...
            // Determine company and employee ID
            if ($user_info['user_type'] == 'staff') {
                $company_id = $user_info['company_id'];
            } else {
                $company_id = $usession['sup_user_id'];
            ...
            $TimesheetModel = new TimesheetModel();
            $employee_id_ok = null; // Initialize employee ID variable

            // Loop through the Excel rows and insert data into the database
            foreach ($sheetData as $row) {
                if (empty($row[0])) continue;  // Skip empty rows

                // Check if the row has the required data for employee info (name and contact number)
                if (!empty($row[0]) && !empty($row[1]) && is_numeric($row[1])) {
                    $user_info = $UsersModel->where('user_id', trim($row[1]))->first();
                    if ($user_info) {
                        $employee_id_ok = $user_info['user_id'];
                        $company_id = $user_info['company_id'];
                    } else {
                        continue;  // Skip to next row if employee not found
                    }
                }

                // Check if the row has the required data for attendance (date, clock-in, clock-out)
                if (!empty($row[0]) && !empty($row[2]) && !empty($row[3]) && $employee_id_ok) {
                    $attendance_date = $this->transformDate($row[0]);
                    $clock_in = $this->transformTime($row[2]);
                    $clock_out = $this->transformTime($row[3]);

                    $existingRecord = $TimesheetModel->where('employee_id', $employee_id_ok)
                        ->where('attendance_date', $attendance_date)
                        ->where('clock_in', $clock_in)
                        ->where('clock_out', $clock_out)
                        ->first();
                    ...
                    $data = [
                        'company_id' => $company_id,
                        'employee_id' => $employee_id_ok,
                        'attendance_date' => $attendance_date,
                        'clock_in' => $clock_in,
                        'clock_out' => $clock_out,
                        'total_work' => $this->transformTime($row[4]),
                        'attendance_status' => 'Present',
                    ];

                    // Insert data into TimesheetModel
                    $TimesheetModel->insert($data);
                }
            }

            $Return['result'] = 'Imported Successfully.';
        } catch (\Exception $e) {
            // Handle errors during Excel reading or processing
