...
class Clients extends BaseController {
	...
	public function leads_list() {
		...
		$data = array();
          ...
          foreach($staff as $r) {						
				...
				$added = $UsersModel->where('user_id', $r['proposed_to'])->first();
				$proposed_to = $added['first_name'].' '.$added['last_name'];

				$added = $UsersModel->where('user_id', $r['added_by'])->first();
				if ($added) {
					$added_by = $added['first_name'].' '.$added['last_name'];
				} else {
					$added_by = '';
				}
	...
	public function leads_followup_list() {
		...
		$data = array();
          ...
          foreach($followup as $r) {
			...
			if($usession['sup_user_id'] == $r['user_id'] || $usession['sup_user_id'] == $r['company_id']){
				$inext_followup = '
				'.$next_followup.'
				<div class="overlay-edit">
					'.$combhr.'
			...
			$added = $UsersModel->where('user_id', $r['user_id'])->first();
	...
	public function add_lead() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$gender = $this->request->getPost('gender',FILTER_SANITIZE_STRING);
			$country = $this->request->getPost('country',FILTER_SANITIZE_STRING);
			$source = $this->request->getPost('source',FILTER_SANITIZE_STRING);
			$lead_description = $this->request->getPost('lead_description',FILTER_SANITIZE_STRING);
			$user_id = $this->request->getPost('user_id',FILTER_SANITIZE_STRING);
			$added_by = $this->request->getPost('added_by',FILTER_SANITIZE_STRING);
