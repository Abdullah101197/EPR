...
jQuery(document).on('change click','.item_name', function () {
	 ...
	 var qty_hrs = jQuery(this).closest('.item-row').find('.qty_hrs').val(1);
	
	 //var eitem_price = qty_hrs * item_price;
...
$(document).ready(function(){
	$('#add-invoice-item').click(function () {
        ...
        jQuery.get(main_url+"purchases/get_purchase_items", function(data, status){
		...
		$.ajax({
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					$('input[name="csrf_token"]').val(JSON.csrf_hash);
					Ladda.stopAll();
					window.location = main_url+'/stock-purchases';
				}
			}
		});
	});	
...
jQuery(document).on('click','.remove-invoice-item-ol', function () {
	...
	$(this).closest('.item-row').fadeOut(300, function() {
	jQuery.get(main_url+"/purchases/delete_invoice_items?record_id="+record_id, function(data, status){
	});
	$(this).remove();
		update_total();
	});
